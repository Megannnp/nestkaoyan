...
- [x] `npm run test:unit` 通过（reducer / replay / plan-generate 等，79/79）
- [x] 关键功能自动化验证：E2E 68/68 PASS（今日工作台 / AI 助手 / 知识中心 / 成长卡片 / 设置 / 复盘 / 真题库 / Reader 阅读 / 上传 / Onboarding / 移动端导航）
...
- [x] 确认当前存储策略：浏览器 localStorage + 本地 SQLite 双写（三种安装方式默认启用，`data/kaoyan.db`）；Cloudflare D1 为云端多端同步可选
